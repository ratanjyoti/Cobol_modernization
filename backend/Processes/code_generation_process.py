...
class CodeGenerationProcess:
    ...
    def validate_generated_project(
        ...
        saved_results, _ = self._load_saved_generation_results(run_id, target.value)
        validation.update(
            self._metadata_from_saved_results(
                run_id=run_id,
                target=target,
    ...
    def _code_generation_metadata(
        ...
        *,
        ...
        agent_name = self._conversion_agent_name(target.value)
        agent_key = target.value
        fallback_used = False
        fallback_reason = ""
        ...
        for result in results:
            notes = []
            for generated_file in result.generated_files:
                notes.extend(generated_file.notes or [])

            for note in notes:
                text = str(note or "")
                if text.startswith("Generated by "):
                    agent_name = text.replace("Generated by ", "").rstrip(".")
                if "fallback" in text.lower():
                    fallback_used = True

            for warning in result.warnings or []:
                warning_text = str(warning or "")
                if "fallback" in warning_text.lower():
                    fallback_used = True
                    fallback_reason = warning_text.split(":", 1)[-1].strip() or fallback_reason

        procedural_flow_used = any(
            bool(self._load_procedural_flow_for_file(context.run_id, context.file_id))
            for context in file_contexts
        )
        business_rules_used = any(
            bool(getattr(context, "business_rules", None))
            for context in file_contexts
        )
        validation_status = (validation or {}).get("status", "PENDING")

        return {
            "conversion_agent": agent_name,
            "conversion_agent_key": agent_key,
            "fallback_used": fallback_used,
            "fallback_reason": fallback_reason,
            "business_rules_used": business_rules_used,
            "procedural_flow_used": procedural_flow_used,
            "quality_gate_status": (quality_report or {}).get("status", "PENDING"),
            "validation_status": validation_status or "PENDING",
        }

    def _metadata_from_saved_results(
        self,
        *,
        run_id: str,
        target: TargetLanguage,
        ...
        source_names = {
            ...
            for result in results
            for generated_file in result.generated_files
            if generated_file.source_file
        }
        file_contexts = []
        try:
            contexts = self.context_builder.build_file_contexts(run_id)
            file_contexts = [
                context
                for context in contexts
                if not source_names or context.filename in source_names
            ]
        except Exception:
            file_contexts = []

        business_rules_used = False
        procedural_flow_used = False
        for result in results:
            for generated_file in result.generated_files:
                for note in generated_file.notes or []:
                    text = str(note or "").lower()
                    if "business rules" in text:
                        business_rules_used = True
                    if "procedural flow used: true" in text:
                        procedural_flow_used = True

        if not business_rules_used and file_contexts:
            business_rules_used = any(bool(context.business_rules) for context in file_contexts)

        metadata = self._code_generation_metadata(
            target=target,
            results=results,
            file_contexts=file_contexts,
            quality_report=quality_report,
        ...
        metadata["business_rules_used"] = metadata["business_rules_used"] or business_rules_used
        metadata["procedural_flow_used"] = metadata["procedural_flow_used"] or procedural_flow_used
        metadata["validation_status"] = (validation or {}).get("status", "PENDING")
        return metadata

    @staticmethod
    def _conversion_agent_name(target_language: str) -> str:
        target = str(target_language or "java").lower().strip()
        if target == "python":
