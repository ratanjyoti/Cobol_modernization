...
class LogicExtractionProcess:
    ...
    def __init__(self, db_session: Session, llm_provider: str | dict, api_key: str = None):
        ...
        self.config = llm_provider if isinstance(llm_provider, dict) else {"mode": llm_provider, "key": api_key}
        self.llm_provider = (self.config.get("mode") or self.config.get("provider") or "local").lower()
        ...
        try:
            ...
            print(f"Client unavailable; using local fallback: {exc}")
    ...
    async def extract_all_rules(self, run_id: str):
        ...
        analysis_map = {analysis.chunk_id: analysis.technical_yaml for analysis in analyses}
        ...
        files = {file.id: file for file in self.db.query(ProjectFile).filter_by(run_id=run_id).all()}
        ...
        for chunk in chunks:
            ...
            use_llm = self.llm_provider in {"openrouter", "api", "custom", "cloud"} and llm_used_count < self.max_llm_chunks
            if use_llm:
                llm_used_count += 1
            ...
            result = self.agent.extract_rules(
                ...
                use_llm=use_llm,
                source_name=project_file.filename,
            )

            prepared_rules.extend(self._prepare_agent_result(result, chunk, technical_yaml))
        ...
        for index, rule in enumerate(prepared_rules, start=1):
            self.db.add(BusinessRule(
                ...
                functional_logic=rule["functional_logic"],
                business_logic=rule["functional_logic"] or rule["rule_text"],
                status="PENDING",
            ))
    ...
    def _prepare_agent_result(self, result, chunk: FileChunk, technical_yaml: str):
        ...
        prepared = []
        for rule in rules:
            rule_text = (rule.get("rule_text") or "").strip() if isinstance(rule, dict) else ""
            if not rule_text:
                continue
            prepared.append({
                "chunk_id": chunk.id,
